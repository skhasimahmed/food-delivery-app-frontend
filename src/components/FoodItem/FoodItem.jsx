...
const FoodItem = ({ id, name, price, description, image, ratings }) => {
  ...
  const handleSubmitRating = async (rating, foodId) => {
    ...
    await axiosInstance.post(
      `${API_BASE_URL}api/user/give-rating/${authUser.userId}`,
      newReview
    )
    .then(response => {
    ...
    <div className="food-item">
      ...
      <div className="food-item-info">
        <div className="food-item-name-rating">
          ...
          {
            [1, 2, 3, 4, 5].map(item => (
              item <= averageRating ? <span className="star" key={item} onClick={() => handleSubmitRating(item, id)}>&#9733;</span> : <span className="star" key={item} onClick={() => handleSubmitRating(item, id)}>&#9734;</span>
              )
            )
            }
          </div>
